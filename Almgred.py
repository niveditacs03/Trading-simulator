...
#almgredchriss model implemented it as a class to import to another file
#same code as in linkedin
class AlmgredChriss:
    def __init__(self, alpha=1.0, beta=1.0, gamma=0.05, eta=0.05, time_steps=50):
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.eta = eta
    ...
    def estimate_impact(self, order_qty, mid_price, volatility):
        # For real-time usage: use a one-step impact estimate or simulate full trajectory
        temp_impact = self.eta * (order_qty / self.time_steps) ** self.alpha
        perm_impact = self.gamma * (order_qty / self.time_steps) ** self.beta
        exec_risk = 0.5 * (volatility ** 2) * (order_qty ** 2)

        # Estimated total cost from Almgren-Chriss
        return mid_price * (temp_impact + perm_impact) + exec_risk
